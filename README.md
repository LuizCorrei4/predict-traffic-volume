...
### 4. Final Evaluation (Test Mode / The Vault)
*(Execute this ONLY once you have decided on the absolute best model).*
Train on the concatenated Train + Validation sets and evaluate once on the untouchable Test Set. For instance, executing 150 tuning trials for the Random Forest champion:
```bash
poetry run python src/models/train_model.py --mode test --models random_forest --trials 150
```

### 5. Generate Visualization Reports
Automatically reads the JSON logs generated by the training step and outputs beautiful Matplotlib/Seaborn metric comparisons and Time Series charts (Actual vs Predicted).
```bash
poetry run python src/models/plot_metrics.py --mode test
```
*   **Note:** The plotting script extracts full range, month, week, and **3-day (micro-trend)** snapshots to visualize how the model captures high-frequency real-world traffic patterns. All images are saved to `reports/figures/`.
